...
contract Loan is MetaCoin
{
    ...
    function mull(uint x, uint y) public pure returns(uint)
    {
        return x * y;
    }
    ...
    function getCompoundInterest(uint256 principle, uint rate, uint time) public pure returns(uint256)
    {
        ...
        for (uint i = 0; i < time; i = i + 1)
        {
            uint x = mull(rate, principle) / 100;
            principle = add(principle, x);
        }
        return principle;
    }
    ...
    function viewDues(address _from) public view isOwner returns(uint256)
    {
        return loans[_from];
    }
    function settleDuesDues(address _to) public isOwner returns(bool)
    {
        if (loans[_to] <= getOwnerBalance())
        {
            sendCoin(_to, loans[_to], Owner);
            loans[_to] = 0;
            return true;
        }
        else
